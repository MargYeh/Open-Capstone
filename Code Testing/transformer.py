...
def get_configs(file_path):
    ...
    try:
        ...
        STORAGE_ACCOUNT = config_parser.get('settings','STORAGE_ACCOUNT')
        STORAGE_KEY = config_parser.get('settings', 'STORAGE_KEY')
        CONTAINER = config_parser.get('settings', 'CONTAINER')
...
if __name__ == "__main__":
    ...
    for prefix, df in transformer.dataframes.items():
        parquet_path = f"{url}/data/{prefix}_combined.parquet"
        csv_path = f"{url}/data/{prefix}_combined.csv"
        
        df.write.mode("overwrite").parquet(parquet_path)
        df.write.mode("overwrite").csv(csv_path)

        logger.info(f"Saved combined DataFrame for {prefix} to {parquet_path} (Parquet) and {csv_path} (CSV)")
